  ...
  `),n.document.close()}function y(e){return String(e??``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`)}function b(e,t,n){t?(e.disabled=!0,e.innerHTML=`<span class="spinner"></span> Please wait...`):(e.disabled=!1,e.innerHTML=n)}function x(e,t=`info`){let n={info:`#6366f1`,success:`#10b981`,error:`#ef4444`,warning:`#f59e0b`},r=document.createElement(`div`);r.style.cssText=`position:fixed;bottom:1.5rem;right:1.5rem;z-index:9999;padding:0.75rem 1.25rem;border-radius:0.5rem;color:white;font-size:0.875rem;font-weight:500;background:${n[t]||n.info};box-shadow:0 10px 40px rgba(0,0,0,0.4);max-width:320px;transition:opacity 0.4s;`,r.textContent=e,document.body.appendChild(r),setTimeout(()=>{r.style.opacity=`0`,setTimeout(()=>r.remove(),400)},3500)}async function S(e){e.innerHTML=`<div class="min-h-screen flex items-center justify-center"><span class="spinner"></span></div>`;let t=new Date().getFullYear();try{let e=await u.getPublicSchedule();e.electionYear&&(t=e.electionYear)}catch{}e.innerHTML=`
    <div class="page-enter min-h-screen flex flex-col">
      <header class="glass sticky top-0 z-50 border-b border-white/10">
        <div class="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div class="flex items-center gap-3">
            ...
            <h1 class="text-xl font-bold text-white tracking-tight">GVC Election Portal ${t}</h1>
          </div>
      ...
      <footer class="py-12 border-t border-white/5 text-center text-slate-500 text-sm">
        <p>&copy; ${t} Government Victoria College, Palakkad</p>
    ...
    `),t.document.close()},i=async(e=`all`)=>{let n,r,i;try{[n,r,i]=await Promise.all([u.adminGetPosts(t),u.getFinalNominations(),u.getPublicSchedule()])}catch(e){throw Error(e.message.includes(`not published`)?`Final List Not Published. Please finalize and publish the list first.`:e.message)}let a=i.electionYear||new Date().getFullYear().toString(),o=r.active||[];if(o.length===0)throw Error(`No active candidates found.`);let s=e=>{let t=e.post.toLowerCase();return t.includes(`representative`)||t.includes(`year`)},c=e=>{let t=e.post.toLowerCase();return t.includes(`association`)||t.includes(`assoc`)},l=e=>!s(e)&&!c(e),d=n.filter(e=>o.filter(t=>t.post===e.post).length>1),f=``;if(e===`all`||e===`general`){let e=d.filter(l);if(e.length>0){f+=`
          <div class="ballot-container a3 page-break">
            ...
            <div style="border-bottom: 2px dotted #000; padding-bottom: 20px; margin-bottom: 30px; text-align: center;">
              <h1 style="font-size: 16px; margin: 0;">COLLEGE UNION ELECTION ${a}</h1>
            ...
            <div class="ballot-header">
              <h1>COLLEGE UNION ELECTION ${a}</h1>
        ...
        `}),f},a=async e=>{try{x(`Generating ballots...`,`info`),r(await i(e))}catch(e){x(e.message,`error`)}},o=async()=>{try{x(`Calculating ranges...`,`info`);let[e,n,i,a,o]=await Promise.all([u.adminGetBooths(t),u.getNominalRoll(),u.adminGetPosts(t),u.getFinalNominations(),u.getPublicSchedule()]),s=a.active||[],c=o.electionYear||new Date().getFullYear(),l=e=>e.post.toLowerCase().includes(`representative`)||e.post.toLowerCase().includes(`year`),d=e=>e.post.toLowerCase().includes(`association`)||e.post.toLowerCase().includes(`assoc`),f=1,p=1,m=1,h=[...e].sort((e,t)=>e.boothNumber-t.boothNumber),g=[],_=[],v=[];h.forEach(e=>{let t=n.filter(t=>e.classes.includes(String(t.CLASS).trim()));t.length!==0&&(g.push({booth:e.boothNumber,count:t.length,start:f,end:f+t.length-1}),f+=t.length)}),i.filter(l).filter(e=>s.filter(t=>t.post===e.post).length>1).forEach(e=>{let t=e.yearRestriction,r=[];h.forEach(i=>{let a=n.filter(e=>i.classes.includes(String(e.CLASS).trim())).filter(e=>{let n=String(e.CLASS).toUpperCase();return n.includes(`PH D`)||n.includes(`PH.D`)?!1:t===`PG`?[`MA`,`MSC`,`MCOM`].some(e=>n.includes(e)):n.includes(t+`ST YEAR`)||n.includes(t+`ND YEAR`)||n.includes(t+`RD YEAR`)||n.includes(t+` YEAR`)});a.length>0&&(r.push({booth:i.boothNumber,post:e.post,count:a.length,start:p,end:p+a.length-1}),p+=a.length)}),r.length>0&&_.push(...r)}),i.filter(d).filter(e=>s.filter(t=>t.post===e.post).length>1).forEach(e=>{let t=e.post.replace(`Association Secretary `,``).trim().toUpperCase(),r=[];h.forEach(i=>{let a=n.filter(e=>i.classes.includes(String(e.CLASS).trim())).filter(e=>String(e.Dept||``).trim().toUpperCase()===t);a.length>0&&(r.push({booth:i.boothNumber,post:e.post,count:a.length,start:m,end:m+a.length-1}),m+=a.length)}),r.length>0&&v.push(...r)}),r(`
...
Are you absolutely sure?`))return;b(t,!0,`🗑️ Wiping...`);let i=n.querySelector(`#wipeStatus`);i.innerHTML=``;try{await u.adminWipeData(r),n.querySelector(`#wipePasswordInput`).value=``,i.innerHTML=`
  ...
  `);try{let[n,r,i,a]=await Promise.all([u.getPosts(),u.getFinalNominations(),u.getResults().catch(()=>[]),u.getPublicSchedule().catch(()=>({}))]);Le(e.querySelector(`#adminMain`),t,n,r.active||[],i,a)}catch(t){e.querySelector(`#adminMain`).innerHTML=`<div class="alert alert-error">❌ ${y(t.message)}</div>`}}}function Le(e,t,n,r,i,a){let o=a.electionYear||new Date().getFullYear(),s={};i.forEach(e=>{s[e.Post]||(s[e.Post]={}),s[e.Post][e.CandidateId]||(s[e.Post][e.CandidateId]=0),s[e.Post][e.CandidateId]+=Number(e.Votes)||0});let c=n.map(e=>{let t=r.filter(t=>t.post===e.post),n=s[e.post]||{};if(t.length===1)return{post:e.post,type:`unanimous`,winner:t[0],candidates:t};if(t.length===0)return{post:e.post,type:`no-candidates`};let i=t.map(e=>({...e,votes:n[e.id]||0}));i.sort((e,t)=>t.votes-e.votes);let a=i[0].votes,o=i.filter(e=>e.votes===a&&e.votes>0),c=o.length>1;return{post:e.post,type:`election`,candidates:i,winner:c?null:o[0],isTie:c,totalVotes:Object.values(n).reduce((e,t)=>e+t,0),nota:n.NOTA||0,invalid:n.INVALID||0}});e.innerHTML=`
    <div class="page-enter space-y-6">
