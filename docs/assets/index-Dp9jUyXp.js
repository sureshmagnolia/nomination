...
`);var $=e=>t=>{Q.innerHTML=``,e(Q,t)};a.on(`/`,$(o)).on(`/submit`,$(E)).on(`/find`,$(ie)).on(`/valid-list`,$(N)).on(`/final-list`,$(I)).on(`/withdraw`,$(z)).on(`/admin`,$(U)).on(`/admin/dashboard`,$(ue)).on(`/admin/verify`,$(de)).on(`/admin/withdrawals`,$(pe)).on(`/admin/publish`,$(he)).on(`/admin/posts`,$(ge)).setDefault(`/`),document.addEventListener(`click`,e=>{let t=e.target.closest(`[data-nav]`);t&&(e.preventDefault(),a.navigate(t.dataset.nav))}),a.start();