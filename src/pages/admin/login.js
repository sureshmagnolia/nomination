...
export function renderAdminLogin(container) {
  ...
  container.querySelector('#btnVerifyLogin').onclick = async (e) => {
    ...
    try {
      const verifyRes = await api.adminVerifyOTP(password, otp);
      ...
      if (verifyRes.sessionToken) {
        localStorage.setItem('adminSessionToken', verifyRes.sessionToken);
        sessionStorage.setItem('adminSessionToken', verifyRes.sessionToken);
      }
